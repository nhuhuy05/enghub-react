...
import type { PreviewResult, PublishResult, Test } from '../../types/adminTestTypes';
...
export const StepPublish = ({ testId, prevStep, onComplete, isEditLocked = false, onStatusChange }: StepPublishProps) => {
  ...
  const loadStatus = async (clearMessages = true) => {
    try {
      ...
      const [testRes, previewRes] = await Promise.all([
        adminTestService.getTestById(testId),
        adminTestService.previewTest(testId),
      ]);
  ...
  const handlePublish = async () => {
    try {
      ...
      const previewRes = await adminTestService.previewTest(testId);
      ...
      const res = await adminTestService.publishTest(testId);
      if (res.code !== 1000) throw new Error(res.message || 'Xuất bản thất bại.');
      setPublishResult(res.result);

      const testRes = await adminTestService.getTestById(testId);
      if (testRes.code === 1000) {
        setTest(testRes.result);
        onStatusChange?.(testRes.result);
      }
  ...
  const handleUnpublish = async () => {
    try {
      ...
      const res = await adminTestService.unpublishTest(testId);
