...
import type { TestCollection, Test } from '../types/adminTestTypes';
...
export const TestListPage = () => {
  ...
  const fetchCollections = async () => {
    try {
      ...
      const res = await adminTestService.getCollections();
      if (res.code === 1000) {
  ...
  const fetchTests = async (collectionId: number) => {
    try {
      ...
      const res = await adminTestService.getTestsInCollection(collectionId);
      if (res.code === 1000) {
  ...
  const handleCreateCollection = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg('');
      const res = await adminTestService.createCollection({
