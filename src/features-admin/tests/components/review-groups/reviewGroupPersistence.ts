...
import type { PatchGroupPassageInput, QuestionGroupDetail } from '../../types/adminTestTypes';
...
export const persistDirtyReviewGroupDetail = async (
  ...
  if (dirtySnapshot.images) {
    const res = await adminTestService.patchGroupImages(
      snapshot.id,
  ...
  if (dirtySnapshot.audio) {
    const res = await adminTestService.patchGroupAudio(snapshot.id, {
  ...
  if (dirtySnapshot.passages) {
    ...
    const res = await adminTestService.patchGroupPassages(snapshot.id, passages);
  ...
  for (const questionId of dirtySnapshot.questionIds) {
    ...
    const res = await adminTestService.patchQuestion(question.id, {
  ...
  for (const answerId of dirtySnapshot.answerIds) {
    ...
    if (!answer) continue;
    const res = await adminTestService.patchAnswer(answer.id, {
  ...
  if (snapshot.review_status === 'reviewed') {
    const res = await adminTestService.patchReviewStatus(snapshot.id, 'needs_review');
