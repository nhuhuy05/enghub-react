    ...
    {tokens.map(({ token, tokenIndex, wordIndex }) => {
      ...
      return shouldHide ? (
        <button
          ...
          className="inline-flex min-h-8 min-w-[54px] rounded-md bg-[#dfe5ee] transition hover:bg-[#cbd5e1] focus:outline-none focus:ring-2 focus:ring-[#0ea5e9]"
          aria-label={`Reveal ${token}`}
        />
      ) : (
        <span key={`${token}-${tokenIndex}`} className={isRevealed ? 'rounded-md bg-emerald-100 px-2.5 py-1.5 text-emerald-800' : ''}>
          {token}
        </span>
      );
    })}
