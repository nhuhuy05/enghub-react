...
import { BookOpenText, BookText, ChevronLeft, ChevronRight, FileSpreadsheet, Headphones, LogOut, Users } from 'lucide-react';
...
const navItems = [
  ...
  { label: 'Đề thi', href: '/admin/tests', icon: FileSpreadsheet },
  { label: 'Luyện nghe', href: '/admin/listening', icon: Headphones },
  { label: 'Luyện đọc', href: '/admin/reading', icon: BookOpenText },
...
export const AdminLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fb] text-[#101828]">
      <aside
        className={`fixed inset-y-0 left-0 hidden border-r border-[#d8dced] bg-white px-4 py-5 transition-all duration-200 lg:flex lg:flex-col ${
          isCollapsed ? 'w-20' : 'w-64'
        }`}
      >
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
          <EngHubLogo markClassName="h-8 w-11" textClassName={isCollapsed ? 'hidden' : 'text-2xl'} />
          {!isCollapsed && (
            <button
              type="button"
        ...
        <button
          ...
          className={`mt-auto flex items-center rounded-lg px-3 py-2 text-sm font-bold text-red-600 transition hover:bg-red-50 ${
            isCollapsed ? 'justify-center' : 'gap-3'
          }`}
      ...
      <main className={`min-h-screen transition-all duration-200 ${isCollapsed ? 'lg:pl-20' : 'lg:pl-64'}`}>
        <div className="flex items-center justify-between border-b border-[#d8dced] bg-white px-4 py-4 lg:hidden">
          <EngHubLogo markClassName="h-8 w-11" textClassName="text-2xl" />
          <button
            type="button"
            onClick={() => void handleLogout()}
            className="inline-flex items-center gap-2 rounded-lg border border-red-100 px-3 py-2 text-sm font-bold text-red-600"
          >
