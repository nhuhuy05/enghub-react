...
export const AnswerTokens = ({ tokens, translation }: AnswerTokensProps) => (
  <div className="mt-3 rounded-xl bg-[#f6f7fc] p-3">
    <p className="mb-2 text-xs font-black uppercase tracking-wide text-[#667085]">Đáp án</p>
    {tokens.map((token, index) => (
      <span key={`${token}-${index}`} className="mr-1.5 inline-flex rounded-md bg-yellow-100 px-2.5 py-1.5 text-sm font-semibold text-orange-700">
        {token}
      </span>
    ))}
    <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-700">
      ...
      <p className="mt-0.5 font-medium">{translation}</p>
    </div>
  </div>
);
