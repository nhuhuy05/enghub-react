        ...
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          ...
          <div className="relative flex-1 lg:max-w-md">
            ...
            <input
              ...
              className="w-full rounded-xl border border-[#e2e8f0] bg-white py-2.5 pl-10 pr-4 text-sm outline-none transition-all focus:border-[#004ac6] focus:ring-2 focus:ring-blue-100"
            />
          </div>
        </div>
      </section>
      ...
      {isLoading ? (
        ...
        <section className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {filteredTopics.map((topic) => (
            <article
              key={topic.id}
              className="group relative flex flex-col rounded-3xl border border-[#e2e8f0] bg-white p-6 transition-all hover:border-[#004ac6] hover:shadow-xl hover:-translate-y-1"
            >
              {topic.isNew && (
